...
import gc
import os
from myutils_stanford import get_ap_score
import numpy as np
import logging
import time
...
def train_model(model, device, criterion, optimizer, lr_annealing, train_data, eval_metric, epoch, epoch_size): #, scheduler
    ...
    for i, batch in enumerate(train_data):
        ...
        if (i + 1) % update_interval == 0:
            ...
            print(f'[{epoch + 1}, {i + 1:5d}] loss: {running_loss / update_interval:.3f}')
            running_loss = 0.0
...
def validate(model, val_data, eval_metric, device):
    ...
    with torch.no_grad():
        for batch in val_data:
            cls_scores = []
            gt_classes = []
            for data, label, box, fixations in zip(*batch):
                data, label, box, fixations = data.to(device), label.to(device), box.to(device), fixations.to(device)
