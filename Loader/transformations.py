...
def fix_resize(fix, in_size, out_size):
    """Resize bouding boxes according to image resize operation.
...
def fix_flip(fix, size, flip_x=False, flip_y=False):
    """Flip bounding boxes according to image flipping directions.
    ...
    width, height = size
    fix = fix.copy()
    nonzero_ids = (fix[:,1] != 0).astype(int)
    # print(flip_y)
    # print(flip_x)
    if flip_y:
        
        ymax = (height - fix[:, 1])*nonzero_ids
        fix[:, 1] = ymax
        
    if flip_x:
        xmax = (width - fix[:, 0])*nonzero_ids
        fix[:, 0] = xmax
...
class DCNDefaultTrainTransform(object):
    ...
    def __call__(self, src, label, box, fix_box):
        ...
        fixbox = fix_resize(fixbox, (w, h), (img.shape[1], img.shape[0]))
        ...
        fixbox = fix_flip(fixbox, (w, h), flip_x=flips[0])
        ...
        # img = gdata.Normalize()
        return img, bbox.astype('float32'), objbox.astype('float32'), fixbox.astype('float32')


class DCNDefaultValTransform(object):
