...
class VOCAction(VisionDataset):
    ...
    def __init__(self, root=os.path.join('~', 'data', 'VOCdevkit(test)'), split='train', index_map=None, preload_label=True, augment_box=False, load_box=False, random_cls=False,transform = None):
        ...
        self._fix_box_path = os.path.join(self._root, 'Boxes_Allscan_test', '{}.pkl')
    ...
    def __getitem__(self, idx):
        ...
        if self._load_box:
            ...
            fix_box_path = self._fix_box_path.format(fid)    
            with open(fix_box_path, 'rb') as f1:
                fix_box = pkl.load(f1)
                seq_len = fix_box.shape[0]
                # new_fix_box = np.zeros((seq_len,4))
                
    
                # new_fix_box[:,0:2] = np.subtract(fix_box[:,0:2],np.tile(np.array([h_x/5, h_y/5]),(seq_len,1)))
                # new_fix_box[:,2:4] = np.add(fix_box[:,0:2],np.tile(np.array([h_x/5, h_y/5]),(seq_len,1)))
                # zero_row = np.where(~fix_box.any(axis=1))[0]
                # new_fix_box[zero_row,:] = 0
                # print(new_fix_box)

            if self.transform:
                img,label,box, fix_box = self.transform(img,label,box, fix_box)
            return img, label, box, fix_box, img_id
    ...
    def _load_items(self, split):
        ...
        ids = []
        set_file = os.path.join(self._root, 'ImageSets_mod', 'Action', split + '.txt')
