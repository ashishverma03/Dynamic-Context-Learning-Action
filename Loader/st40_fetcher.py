...
class Stanford40Action(VisionDataset):
    ...
    def __init__(self, root=os.path.join('~', 'data', 'Stanford40'), split='train', index_map=None, preload_label=True, augment_box=False, load_box=False,transform = None):
        ...
        self._fix_box_path = os.path.join(self._root, 'Predicted_Boxes_Allscan', '{}.pkl')
        # self._hand_box_path = os.path.join(self._root, 'Predicted_Boxes_Hand', '{}.pkl')
    ...
    def __getitem__(self, idx):
        ...
        if self._load_box:
            box_path = self._box_path.format(img_id)
            # hand_box_path = self._hand_box_path.format(img_id)
            with open(box_path, 'rb') as f:
                box = pkl.load(f)
                # print(box)

            # with open(hand_box_path, 'rb') as f0:
            #     hand_box = pkl.load(f0)

            # if hand_box != []:
            #     box = np.concatenate((box, hand_box), axis=0)
            #     # print(box)


            fix_box_path = self._fix_box_path.format(img_id)    
