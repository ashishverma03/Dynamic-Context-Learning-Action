...
import os
import time
from myutils_stanford import encode_labels,plot_history
from Network.training_stanford import train_model , validate, val_model

from torch.utils.data import DataLoader
from Loader import metric,st40_fetcher


from Loader.transformations import DCNDefaultTrainTransform,DCNDefaultValTransform
import torchvision

from Network import network_res_lstm_412_stanford
...
if __name__ == '__main__':
    ...
    # Dataloader
    train_dataset = st40_fetcher.Stanford40Action(split='train', augment_box=True, load_box=True,transform=DCNDefaultTrainTransform(600,1000))
    val_dataset = st40_fetcher.Stanford40Action(split='test', load_box=True,transform=DCNDefaultValTransform(600,1000))
    ...
    eval_metric = metric.VOCClsMApMetric(class_names=val_dataset.classes)

    lr_annealing = metric.CosineAnnealingSchedule(min_lr=1e-6, max_lr=4e-5, cycle_length=30000)
    ...
    #Model
    net = network_res_lstm_412_stanford.Custom_Model(pretrained=True,device=device).to(device)
    criterion = torch.nn.CrossEntropyLoss(reduction='mean')

    optimizer = torch.optim.Adam(net.parameters(), lr=4e-5, betas=(0.9, 0.98))
